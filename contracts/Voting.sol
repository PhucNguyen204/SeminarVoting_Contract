...
contract Voting is Initializable, AccessControlUpgradeable {
        ...
        onlyRole(VOTER_ROLE)
    {
        ...
        require(!round.checkVotedSeminar[seminarId][msg.sender]);
    ...
    function voteForSpeaker(
        ...
        require(!round.checkVotedSpeaker[speaker][msg.sender]);
    ...
    function getSeminarsHaveVoted(
        uint256 roundId
    ) public view onlyRole(VOTER_ROLE) returns (uint256[] memory) {
        uint256 count = 0;
        address voter = msg.sender;
        VotingRound storage round = votingRounds[roundId];
        for (uint256 i = 0; i < round.seminarIds.length; ++i) {
            if (round.checkVotedSeminar[round.seminarIds[i]][voter]) {
                ++count;
            }
        }
        uint256[] memory seminars = new uint256[](count);
        for (uint256 i = 0; i < round.seminarIds.length; ++i) {
            if (round.checkVotedSeminar[round.seminarIds[i]][voter]) {
                --count;
                seminars[count] = round.seminarIds[i];
            }
        }
        return seminars;
    }

    function getSpeakersHaveVoted(
        uint256 roundId
    ) public view onlyRole(VOTER_ROLE) returns (address[] memory) {
        uint256 count = 0;
        address voter = msg.sender;
        VotingRound storage round = votingRounds[roundId];
        for (uint256 i = 0; i < round.speakersInRound.length; ++i) {
            if (round.checkVotedSpeaker[round.speakersInRound[i]][voter]) {
                ++count;
            }
        }
        address[] memory seminars = new address[](count);
        for (uint256 i = 0; i < round.speakersInRound.length; ++i) {
            if (round.checkVotedSpeaker[round.speakersInRound[i]][voter]) {
                --count;
                seminars[count] = round.speakersInRound[i];
            }
        }
        return seminars;
    }
    ...
    function getInActiveRounds() public view returns (uint256[] memory) {
        uint256 count = 0;
        uint256 T = block.timestamp;
        for (uint256 roundId = 1; roundId <= nextRoundId; ++roundId) {
            if (
                votingRounds[roundId].isActive &&
                votingRounds[roundId].startTime <= T &&
                T <= votingRounds[roundId].endTime
            ) {
                ++count;
            }
        }
        uint256[] memory inActiveRounds = new uint256[](count);
        uint256 i = 0;
        for (uint256 roundId = 1; roundId <= nextRoundId; ++roundId) {
            if (
                votingRounds[roundId].isActive &&
                ...
                inActiveRounds[i] = roundId;
                ++i;
            }
        }
        return inActiveRounds;
    }
